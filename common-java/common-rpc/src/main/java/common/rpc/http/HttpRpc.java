...
public class HttpRpc {
    ...
    private <T> T returnOrException(Json<T> json) {
        if (json.getCode() == CodeMessage.OK) {
            return json.getData();
        }

        throw new ServiceException(json.getCode(), json.getMessage());
    }
    ...
    private <T> okhttp3.Callback createCallback(TypeReference<Json<T>> type, Callback<T> callback) {
        return new okhttp3.Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                if (json.getCode() == CodeMessage.OK) {
                    callback.onResponse(call, json.getData());
                } else {
                    callback.onFailure(call, new ServiceException(json.getCode(), json.getMessage()));
                }
            }
        };
    }
